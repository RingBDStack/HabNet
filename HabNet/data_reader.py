...
class DataReader:
  def __init__(self, train_file, dev_file, test_file,
               max_word_length=100, max_sent_length=30, max_review_length=5, num_classes=2):
  ...
  def _read_data(self, file_path):
    ...
    with open(file_path, 'rb') as f:
      data = pickle.load(f)
      random.shuffle(data)
      for label, reviews in data:
        reviews = reviews[:self.max_review_length]
        reviews = [review[:self.max_sent_length] for review in reviews]
        #reviews = [review[sent[:self.max_word_length] for sent in review] for review in reviews]
        for index, review in enumerate(reviews):
          reviews[index] = [sent[:self.max_word_length] for sent in review]

        #label -= 1
        assert label >= 0 and label < self.num_classes

        new_data.append((reviews, label))

    # sort data by review nums to speed up
